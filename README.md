...
- Optional top-level `notes` is an array of nonblank user-written routing preferences, such as `"notes": ["Prefer model X for short frontend tasks"]`. Jev receives these notes when routing; do not put secrets in them.
- Optional `models[].grades` maps skill names to your own integer scores from 1 to 10, such as `"grades": {"frontend": 10, "debugging": 8}`. Use the same skill names without leading or trailing spaces across models for useful comparisons. Missing grades mean unknown, not zero. Notes and grades guide Jev but do not override explicit caller constraints, model availability, or task suitability. Empty `notes` and `grades` are allowed; existing configs need no changes.
